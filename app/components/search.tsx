...
export default function SearchBar() {
  ...
  /**
   * handleSubmit can take optional "overrides" so we don’t rely
   * on possibly stale React state if we call setX + handleSubmit in the same function.
   */
  const handleSubmit = async (overrides?: Partial<SearchParams>) => {
    // Merge overrides with the existing states.
    ...
    // Build payload from the final merged values
    ...
    if (searchParams.location.trim()) {
      ...
      payload.range = searchParams.range // add range
            ...
            <Card className="self-center sm:w-full w-[300px] shadow-lg lg:rounded-[4rem]">
              <CardContent className="p-0 items-center">
                <form
                  ...
                  onSubmit={(e) => {
                    e.preventDefault()
                    // If the user explicitly clicks the “Search” button,
                    // we just do a normal handleSubmit with the current state.
                    handleSubmit()
                  }}
                >
                  {/* Name field */}
                      ...
                      <div className="flex-1 flex flex-col p-2 transition-colors py-4 pl-8 lg:pr-8 w-[265px]">
                        ...
                        <Select
                          ...
                          onValueChange={(newCraft) => {
                            // Set craft in state
                            setCraft(newCraft)
                            // Immediately call handleSubmit with the *new* craft
                            handleSubmit({
                              craft: newCraft,
                            })
                          }}
                    ...
                    {location && showRangeSelector && (
                      <div className="absolute left-0 right-0 top-full z-10 bg-white shadow-lg rounded-b-lg p-4 border border-t-0">
                        ...
                        <input
                          ...
                          // Trigger handleSubmit on mouse/touch release,
                          // passing the *new* range override
                  ...
                  >
                    ...
                    {/* Submit button for manual search click */}
                    <div
